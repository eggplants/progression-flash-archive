...
package jp.nium.core.display {
	...
	/**
	 * <span lang="ja">ExDisplayObject クラスは、IExDisplayObject インターフェイスを実装した DisplayObject クラスの基本機能を拡張するための委譲クラスです。
	 * このクラスを実装すると拡張されたイベント機能を持つ EventIntegrator クラスが Mix-in されることになります。</span>
	 * <span lang="en">ExDisplayObject class which implements IExDisplayObject interface is a delegate class to extend the basic function of DisplayObject class.
	 * If implement this class, the EventIntegrator class which has extended event function will be Mixed-in.</span>
	 * 
	 * @example <listing version="3.0" >
	 * </listing>
	 */
	public class ExDisplayObject implements IExDisplayObject {
		
		/**
		 * <span lang="ja">IExDisplayObject インターフェイスを実装した操作対象インスタンスです。</span>
		 * <span lang="en">It is an instance for the operation which implements the IExDisplayObject interface.</span>
		 */
		protected function get target():IExDisplayObject { return _target; }
		private var _target:IExDisplayObject;
		
		/**
		 * target を DisplayObject 型で取得します。
		 */
		...
		public function set id( value:String ):void { _id = ExDisplayCollection.nium_internal::addInstanceAtId( _target, value ); }
		...
		public function set group( value:String ):void { _group = ExDisplayCollection.nium_internal::addInstanceAtGroup( _target, value ); }
		...
		/**
		 * <span lang="ja">新しい ExDisplayObject インスタンスを作成します。</span>
		 * <span lang="en">Creates a new ExDisplayObject object.</span>
		 * 
		 * @param target
		 * <span lang="ja">関連付けたい IExDisplayObject インターフェイスを実装したインスタンスです。</span>
		 * <span lang="en">It is an instance that wants to be related which implements IExDisplayObject interface.</span>
		 */
		public function ExDisplayObject( target:IExDisplayObject ) {
			if ( !( target is DisplayObject ) ) { throw new ArgumentError( ErrorMessageConstants.getMessage( "ERROR_8002" ) ); }
			
			// クラス名を取得する
			_className = ClassUtil.getClassName( target );
			
			// ExDisplayCollection に登録する
			ExDisplayCollection.nium_internal::addInstance( target );
			
			// 初期化する
			_target = target;
			_displayObject = DisplayObject( target );
			
			// EventIntegrator を作成する
			_integrator = new EventIntegrator( target );
			
			// EventIntegrator を初期化する
			_integrator.nium_internal::initialize( {
				addEventListener		:_integrator.addEventListener,
				removeEventListener		:_integrator.removeEventListener,
				hasEventListener		:_integrator.hasEventListener,
				willTrigger				:_integrator.willTrigger,
				dispatchEvent			:_integrator.dispatchEvent
			} );
		}
		...
		nium_internal function initialize( initObject:Object ):void {
			_integrator.nium_internal::initialize( {
				addEventListener		:initObject.addEventListener,
				removeEventListener		:initObject.removeEventListener,
				hasEventListener		:initObject.hasEventListener,
				willTrigger				:initObject.willTrigger,
				dispatchEvent			:initObject.dispatchEvent
			} );
		}
		...
		public function getInstanceById( id:String ):DisplayObject {
			return DisplayObject( ExDisplayCollection.nium_internal::getInstanceById( id ) );
		}
		...
		public function setProperties( props:Object ):DisplayObject {
			ObjectUtil.setProperties( _displayObject, props );
		 ...
		 * <span lang="en">Whether if it removes the information completely.</span>
		 */
		...
		public function toBitmapData( transparent:Boolean = true, fillColor:uint = 0xFFFFFFFF ):BitmapData {
			var bmp:BitmapData = new BitmapData( _displayObject.width, _displayObject.height, transparent, fillColor );
			bmp.draw( _displayObject );
			return bmp;
		}
		...
		public function toString():String {
			return '[' + _className + ' id="' + _id + '" name="' + _displayObject.name + '" group="' + _group + '"]';
		}
	}
}
