...
package jp.nium.core.display {
	...
	/**
	 * <span lang="ja">IDisplayObject インターフェイスは、基本的な表示オブジェクトに必要な機能を実装します。</span>
	 * <span lang="en">The IDisplayObject interface implements the basic function which display objects needs.</span>
	 * 
	...
	public interface IDisplayObject extends IEventDispatcher {
		 ...
		 * @return
		 * <span lang="ja">targetCoordinateSpace オブジェクトの座標系を基準とする、表示オブジェクトの領域を定義する矩形です。</span>
		 ...
		 * 
		 * @param point
		 * <span lang="ja">Point クラスを使って作成されるオブジェクトです。Point オブジェクトは、x および y 座標をプロパティとして指定します。</span>
		 ...
		 * 
		 * @param point
		 * <span lang="ja">Point クラスを使用し、x および y 座標をプロパティとして指定して作成されるポイントの名前または識別子です。</span>
