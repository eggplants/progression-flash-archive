...
package jp.progression.commands {
	...
	/**
	 * <span lang="ja">LoadChildAtAbove クラスは、対象の DisplayObjectContainer インスタンスのディスプレイリストに request プロパティに設定された外部 SWF ファイルを読み込んで追加するコマンドクラスです。
	 ...
	 * 
	 * // LoadChildAtAbove コマンドを作成します。
	 * var com:LoadChildAtAbove = new LoadChild( container, new URLRequest( "external.swf" ), 0 );
	 * 
	 ...
	 * } );
	 * 
	 * // LoadChildAtAbove コマンドを実行します。
	 * com.execute();
	 * </listing>
	 */
	public class LoadChildAtAbove extends Command {
		...
		/**
		 * <span lang="ja">この LoadChildAtAbove オブジェクトに関係した CastLoader オブジェクトです。</span>
		 * <span lang="en"></span>
		 */
		...
		/**
		 * <span lang="ja">新しい LoadChildAtAbove インスタンスを作成します。</span>
		 * <span lang="en">Creates a new LoadChildAtAbove object.</span>
		 * 
		 * @param container
		 ...
		 * <span lang="en"></span>
		 */
		public function LoadChildAtAbove( container:DisplayObjectContainer, request:URLRequest, index:int, initObject:Object = null ) {
			// 引数を設定する
		...
		/**
		 * <span lang="ja">LoadChildAtAbove インスタンスのコピーを作成して、各プロパティの値を元のプロパティの値と一致するように設定します。</span>
		 * <span lang="en">Duplicates an instance of an LoadChildAtAbove subclass.</span>
		 * 
		 * @return
		 * <span lang="ja">元のオブジェクトと同じプロパティ値を含む新しい LoadChildAtAbove インスタンスです。</span>
		 * <span lang="en">A new LoadChildAtAbove object that is identical to the original.</span>
		 */
		public override function clone():Command {
			return new LoadChildAtAbove( _container, _request, _index, this ).setProperties( {
		...
		private function _commandCompleteLoader( e:CommandEvent ):void {
			...
			if ( _container is IExDisplayObjectContainer ) {
				IExDisplayObjectContainer( _container ).addChildAtAbove( _loader, _index );
			}
			else {
