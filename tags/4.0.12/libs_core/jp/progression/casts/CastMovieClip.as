...
package jp.progression.casts {
	...
	/**
	 * <span lang="ja">CastMovieClip クラスは、ExMovieClip クラスの基本機能を拡張し、イベントフローとの連携機能を実装した jp.progression パッケージで使用される基本的な表示オブジェクトクラスです。</span>
	 * <span lang="en"></span>
	 * 
	 ...
	 * </listing>
	 */
	public class CastMovieClip extends ExMovieClip implements ICastObject, IManageable {
		 ...
		 * <span lang="en"></span>
		 */
		public function get self():CastMovieClip { return CastMovieClip( this ); }
		...
		/**
		 * ドキュメントクラスに設定されているかどうかを取得します。
		 */
		private var _isDocumentClass:Boolean = false;
		 ...
		 * <span lang="en"></span>
		 */
		public function CastMovieClip( initObject:Object = null ) {
			// クラスをコンパイルに含める
			...
			// Progression を設定する
			_manager = CastMovieClip.progression_internal::$manager;
			
			// 破棄する
			CastMovieClip.progression_internal::$manager = null;
		}
		...
		public function updateManager():Boolean {
			...
			if ( _manager ) {
				_isDocumentClass = true;
			}
			else {
				_isDocumentClass = false;
		...
		public function dispose():void {
			// ドキュメントクラスであれば
			if ( _isDocumentClass ) { throw new Error( Logger.getLog( L10NNiumMsg.getInstance().ERROR_000 ).toString( "dispose" ) ); }
		...
		private static function _addedToStage( e:Event ):void {
			...
			// 実装する
			var target:CastMovieClip = e.target as CastMovieClip;
			...
			if ( target._executor ) {
				...
				if ( PackageInfo.hasDebugger ) {
					Logger.warn( Logger.getLog( L10NProgressionMsg.getInstance().WARN_003 ).toString( target, "contextMenu" ) );
				}
			}
		}
