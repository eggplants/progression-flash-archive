...
package jp.progression.scenes {
	...
	public class SceneObject extends EventDispatcher implements IIdGroup, IManageable, IExecutable, IDisposable {
		...
		override public function addEventListener( type:String, listener:Function, useCapture:Boolean = false, priority:int = 0, useWeakReference:Boolean = false):void {
			...
			if ( useCapture ) {
				_captureDispatcher.addEventListener.apply( null, arguments );
			}
			else {
				super.addEventListener.apply( null, arguments );
			}
		}
		...
		override public function removeEventListener( type:String, listener:Function, useCapture:Boolean = false ):void {
			...
			if ( useCapture ) {
				_captureDispatcher.removeEventListener.apply( null, arguments );
			}
			else {
				super.removeEventListener.apply( null, arguments );
			}
		}
